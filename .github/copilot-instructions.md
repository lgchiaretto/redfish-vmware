...
### PATCH Behaviour
- `PATCH /Systems/{vm}` with `Boot.*` - translates `BootSourceOverrideTarget` to a VMware boot order via `_BOOT_TARGET_MAP` and calls `vmware_client.set_vm_boot_order()`. Boot order uses real hardware device keys for disk/NIC; CD-ROM uses `BootableCdromDevice` without a device key. Returns the updated full ComputerSystem.
...
3. Force eject sets `connected=False`/`startConnected=False` and uses `wait_for_task_with_questions()` with a 5-second timeout
4. `wait_for_task_with_questions()` monitors `vm.runtime.question` during the task; if a CD ejection confirmation prompt appears (`vim.option.ChoiceOption`), answers it automatically using `question.choice.choiceInfo[0].key` via `vm.AnswerVM()`
...
Handlers inherit `RedfishResponseMixin` from `handlers/response_utils.py`:
```python
self._send_json_response(request_handler, 200, data)
self._send_error_response(request_handler, 404, "System not found")
```
VMware operations return `True/False` - handlers translate to HTTP status codes. Use `wait_for_task()` / `wait_for_task_with_questions()` from `vmware/task_utils.py` for async vSphere tasks.
...
1. OpenShift creates BMH pointing to `http://bastion.example.com:8443/redfish/v1/Systems/{vm_name}`
...
src/
  ...
  handlers/
    ...
    response_utils.py        # RedfishResponseMixin and shared JSON/error helpers
  vmware/
    ...
    task_utils.py            # wait_for_task(), wait_for_task_with_questions()
  auth/
